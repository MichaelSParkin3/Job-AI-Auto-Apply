...
def test_answer_orchestrator_precedence(run_dir: Path) -> None:
    ...
    assert cached_result.source is AnswerSource.CACHED
    assert cached_result.cached_from is AnswerSource.RESUME_FACT
    ...
    assert run_payload["answerPolicy"]["min_confidence"] == pytest.approx(
        policy.min_confidence
    )
    assert run_payload["answerPolicy"]["allow_llm_fallback"] is policy.allow_llm_fallback
    ...
    cached_entry = next(
        entry for entry in run_payload["answers"] if entry["fieldId"] == "linkedinUrl"
    )
    assert cached_entry["source"] == "cached"
    assert cached_entry["cachedFrom"] == "resume_fact"
    artifact_path = run_dir / cached_entry["artifactPath"]
    artifact_payload = json.loads(artifact_path.read_text(encoding="utf-8"))
    assert artifact_payload["source"] == "cached"
    assert artifact_payload["cachedFrom"] == "resume_fact"
